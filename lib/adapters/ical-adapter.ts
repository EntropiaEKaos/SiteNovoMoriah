...
async function fetchCalendar(value:string){
 let u=await safeCalendarUrl(value);
 for(let hop=0;hop<=MAX_REDIRECTS;hop++){
  const res=await fetch(u,{headers:{"user-agent":"MoriahCalendarSync/2.1","accept":"text/calendar,text/plain;q=0.9,*/*;q=0.1"},redirect:"manual",signal:AbortSignal.timeout(10000),cache:"no-store"});
  ...
  const body=await res.text();if(body.length>MAX_BYTES)throw new Error("Calendário excede o limite permitido.");return body;
 }
...
export const icalChannelAdapter:ChannelAdapter={
 ...
 async sync(context):Promise<AdapterSyncResult>{
  ...
  const body=await fetchCalendar(row.importUrl);
