...
export async function createAdminSession(){if(!secret())throw new Error("SESSION_SECRET não configurado.");const expires=Date.now()+1000*60*60*12;const payload=String(expires);(await cookies()).set(COOKIE,payload+"."+sign(payload),{httpOnly:true,secure:process.env.NODE_ENV==="production",sameSite:"lax",path:"/",maxAge:60*60*12})}
