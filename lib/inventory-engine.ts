...
export async function getInventoryConflicts(accommodationId:string,checkIn:Date,checkOut:Date,excludeBookingId?:string):Promise<InventoryConflict[]>{
 ...
 const [external,internal,holds]=await Promise.all([
 ...
 return [...external.map(row=>({source:sourceOf(row.integration.integrationType),provider:row.integration.provider,reference:row.externalUid,startsAt:row.startsAt,endsAt:row.endsAt})),...internal.filter(row=>row.checkIn&&row.checkOut).map(row=>({source:"DIRECT" as const,provider:"MORIAH",reference:row.id,startsAt:row.checkIn!,endsAt:row.checkOut!})),...holds.map(row=>({source:"DIRECT" as const,provider:"MORIAH_HOLD",reference:row.token,startsAt:row.checkIn,endsAt:row.checkOut}))];
}
...
export async function getAvailableAccommodations(checkIn:Date,checkOut:Date,guests=1){
 ...
 const blocked=new Set<string>();external.forEach(x=>{if(x.integration.accommodationId)blocked.add(x.integration.accommodationId)});internal.forEach(x=>{if(x.accommodationId)blocked.add(x.accommodationId)});holds.forEach(x=>blocked.add(x.accommodationId));
