...
export async function quoteAccommodation(accommodationId:string,checkIn:Date,checkOut:Date){
 ...
 const now=new Date();const leadDays=Math.floor((checkIn.getTime()-now.getTime())/86400000);const applicable=rules.filter(r=>(r.daysBeforeMin==null||leadDays>=r.daysBeforeMin)&&(r.daysBeforeMax==null||leadDays<=r.daysBeforeMax));let total=0;const breakdown:{date:string;priceCents:number;override:boolean}[]=[];
 ...
 return {currency:plan.currency,nights,totalCents:total,averageNightCents:Math.round(total/nights),ratePlan:plan.name,breakdown,appliedRules:applicable.map(r=>r.name)};
}
