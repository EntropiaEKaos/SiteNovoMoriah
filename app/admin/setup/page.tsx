...
export default async function SuperAdminSetup({searchParams}:{searchParams:Promise<{error?:string}>}){
  if(await prisma.adminUser.count({where:{role:"SUPERADMIN"}}))redirect("/admin/login");
  const q=await searchParams;
  const message=q.error==="token"?"Token de configuração inicial inválido.":q.error==="user"?"Use de 3 a 40 caracteres: letras, números, ponto, hífen ou sublinhado.":q.error==="match"?"As senhas não coincidem.":q.error==="password"?"A senha precisa ter pelo menos 12 caracteres.":q.error==="create"?"Não foi possível criar o administrador. Tente novamente.":null;
  return <main style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"#111",padding:24}}>
    <form action={bootstrapSuperAdmin} style={{width:"min(460px,100%)",background:"#fff",padding:38,borderTop:"8px solid #ffd400"}}>
      ...
      <p>Esta página funciona somente enquanto não existir nenhum Super Admin e exige o token privado de configuração inicial.</p>
